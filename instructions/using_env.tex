...
\end{monobox}
    ...
    \begin{infobox}
      When using \textbf{WSL}, we recommend to include the \mono{--build-dir /tmp/build} argument.
  ...
  \item With \textbf{Container Engine}:
  \begin{monobox}
cd ~/zephyrproject/zephyr/samples/basic/blinky/
...
\end{monobox}
    ...
    \begin{infobox}
      When using a \textbf{container engine}, setting the build directory to \mono{\~/dev/build} is necessary, otherwise you cannot access the compiled Zephyr binary from outside the container.
    \end{infobox}
\end{itemize}
...
\begin{infobox}
  ...
  \begin{center}
    \begin{tikzpicture}
      \definecolor{gray}{rgb}{.851,.851,.851}
      \node [anchor=south west] {\includegraphics[width=.5\paperwidth]{explorer_wsl_hex}};
      \draw (.9,1.75) [gray,fill] rectangle (3.5,2.05);
      \node at (.8,1.9) [anchor=west] {\fontsize{6pt}{6pt}\selectfont\imagename};
    \end{tikzpicture}
  \end{center}
