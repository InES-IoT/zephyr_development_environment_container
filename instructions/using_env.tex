...
\end{monobox}
  \item For \textbf{Container Engine}:
  \begin{monobox}
cd ~/zephyrproject/zephyr/samples/basic/blinky/
...
\begin{infobox}
  When using a container engine, setting the build directory to \mono{\~/dev/build} is necessary, otherwise you cannot access the binary from outside the container.
\end{infobox}
