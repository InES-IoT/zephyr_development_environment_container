...
\end{lstlisting}
  ...
  \item Create a folder for the environment and import the provided WSL image.
        \begin{lstlisting}
mkdir choose\your\path\wslDistroStorage\@\imagename{}@
wsl --import @\imagename{}@ `
  your\path\wslDistroStorage\@\imagename{}@ `
...
\begin{infobox}
  When using WSL make sure to always include the \mono{--build-dir /tmp/build} argument.
...
If a J-Link command file (usually located at \mono{../jlink/<name>.jlink}) is present in the project, it can be used to flash the binary from the command line.
...
\begin{lstlisting}
$env:Path += ";C:\Program Files\SEGGER\JLink_V796c"
JLink -CommandFile path\to\file.jlink
\end{lstlisting}

\begin{infobox}
  On your system, the path to the J-Link executable might be different.
  Use the file explorer to find the right path.
\end{infobox}

\begin{infobox}
  The above command to add the path is only for the current session. You can also add it permanently if you want.
\end{infobox}
